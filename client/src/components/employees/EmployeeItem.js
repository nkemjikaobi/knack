    ...
    const { setCurrent } = EmployeeContext;
        ...
        return (
            ...
            <div className='card text-center m-3'>
                ...
                <div>
                    <button className={`btn ${employee.isAvailable ? 'green' : 'red'} btn-sm m-1`}>
                    ...
                    <a href="#schedule-meeting-modal" onClick={() => setCurrent(employee)} className={`btn blue btn-sm my-1 modal-trigger meeting`} disabled={!employee.isAvailable}>{employee.isAvailable ? 'Schedule Meeting' : 'Cannot Schedule'} <i className="far fa-calendar-alt"></i></a>
                </div>
            </div>
        )
}
